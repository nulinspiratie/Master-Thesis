...
\part{Muxmon experiment}
  ...
  \chapter{Exploring frequency re-use}
    ...
    \section{Selective broadcasting}
      \label{sec:selective broadcasting}
      A second challenge that exists when scaling up is that as the number of qubits grow, so do the instruments needed to control them. It is unlikely
...
In the Muxmon set-up two of the four input ports are used. One input port receives the main pulse, the other receives the DRAG pulse. These are combined in the Duplexer. The output ports are connected to the drive lines of the top and bottom qubit.
