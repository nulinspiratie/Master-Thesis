  ...
  \chapter{Qubit characterization}
    ...
    \section{Continuous-wave measurements}
      ...
      \subsection{Finding the qubit's second transition}
        Once the qubit's grequency is known, it is possible to find the qubit's excited-state to second-excited-state transition $\omega_q^{12}$, which shall be referred to as the 12-transition. The anharmonicity $\alpha$ is then given by the difference in transition frequencies: $\alpha = \omega_q^{12} - \omega_q$.
        ...
        \begin{enumerate}
          \item A drive tone with varying frequency frequency $\omega_\text{drive}^{12}$
          \item A second drive tone with fixed frequency at the qubit frequency $\omega_\text{drive}^{01} = \omega_\text{drive}$
          \item A hird tone at the resonator frequency when the qubit is in the ground-state
        \end{enumerate}



        \begin{itemize}
           \item Coupling energy.
         \end{itemize}
