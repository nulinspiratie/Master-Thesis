    ...
    \section{Time-domain measurements}
      ...
      \subsection{Qubit decoherence}
        ...
        \subsubsection{Echo measurement}
          ...
          An Echo measurement is performed by sending two $X_{\pi/2}$ pulses separated by a wait time $\tau$. In the middle of this wait time, at $\tau/2$, an additional refocusing $X_{\pi}$ pulse is sent. An Echo measurement is quite similar to a Ramsey measurement, but the crucial difference is the refocusing $X_{\pi}$ pulse. This has th

           that in the middle of this wait time, at $\tau/2$, an $_{\pi}$ pulse is applied, and the final pulse is an $X_{\pi/2}$ pulse instead of an $X_{-\pi/2}$.


is quite similar to a Ramsey measurement, where an $X_{\pi/2}$ pulse is sent, and after a wait time $\tau$ an $X_{-\pi/2}$ pulse is sent.
          \begin{itemize}
