  ...
  \chapter{Exploring frequency re-use}
    ...
    \section{frequency re-use}
      \begin{wrapfigure}[8]{r}{0.45\textwidth}
        \begin{center}
        \vspace{-30pt}
          \includegraphics[width=.5\textwidth]{../Figures/Exploring frequency re-use/bandwidth_broadening.png}
        \end{center}
        \vspace{-5pt}
      ...
      \begin{figure}[tb]
        \centering
        \includegraphics[width=0.6\textwidth]{../Figures/Exploring frequency re-use/surface_code_frequency re-use.jpg}
      ...
      Frequency re-use does pose potential issues. Two unwanted effects in particular arise when multiple qubits that are directly or indirectly coupled to each other share the same frequency. The first effect is cross-coupling, where an excitation can be transferred from one qubit to the other. This will be discussed in Section~\ref{sec:cross-coupling}. The second effect is cross-driving, where a pulse partially leaks through the components, and will . This leakage of a pulse is always present, but does not pose a problem as long as the frequency of the pulse is sufficiently separated from the frequency of the other qubits.
    ...
    \section{Direct driving versus capacitive driving}
      ...
      \centering
        ...
        \end{subfigure}
        \caption[Muxmon chips]{The Muxmon0 and Muxmon1 chips. The qubits of Muxmon0 have direct drive lines, while the }
    ...
    \section{Duplexer}
      ...
      The Duplexer shown in Figure~\ref{fig:Duplexer picture} is a patented vector switch matrix designed by Duije Deurloo, who is working at TNO. It has four input ports and two ouput ports. Each of the input-output port combinations has the following successive components:
      ...
      \caption[Two numerical solutions]{(a) drive amplitude, (b) DRAG amplitude}
      \label{fig:AllXY individual control}
      \end{figure}

      The result of these components is that the signal of each input-output port combination can be individually modified. In the Muxmon set-up two of the four input ports are used. One input port receives the main pulse, the other receives the DRAG pulse. These are combined in the Duplexer. The output ports are connected to the drive lines of the top and bottom qubit. In Figure~\ref{fig:AllXY individual control} the AllXY results are shown for the top and bottom qubits, as the drive amplitude and DRAG parameter of the top qubit is varied using the Duplexer. As can be seen there is no noticeable change in the AllXY of the bottom qubit, even though the top qubit's performance changes considerably. This shows that the Duplexer allows for individual drive amplitude and DRAG parameter control for each qubit separately.

      The Duplexer's digital switches have an RF-switching time of \SI{4}{\nano \second}. Using these switches, pulses may be sent to either of the two qubits, or to both qubits simultaneously. This allows for selective broadcasting. This property will be exploited in Randomized Benchmarking in Chapter~\ref{ch:randomized benchmarking}.
