    ...
    \section{Time-domain measurements}
      ...
      \subsection{Qubit decoherence}
        ...
        \subsubsection{Echo measurement}
          ...
          An Echo measurement is performed by sending two $X_{\pi/2}$ pulses separated by a wait time $\tau$. In the middle of this wait time, at $\tau/2$, an additional refocusing $X_{\pi}$ pulse is sent. The Echo sequence is quite similar to the Ramsey sequence, but the crucial difference is the refocusing $X_{\pi}$ pulse. This has the effect that the state of the qubit is essentially flipped on the Bloch sphere around the X-axis. Any slow phase noise, which we can view to be quasi-static, is thereby cancelled. Fast phase noise, however, will vary considerably during the wait time $\tau$, and so will still cause dephasing.

          An Echo measurement is performed by monotonically increasing the wait-time


          \begin{itemize}
            \item How to distinguish both effects? Echo measures fast, is everything else slow? How is this quantified?
            \item Refocusing
            \item quasi-static noise
            \item can also use multiple refocusing pulses
          \end{itemize}
