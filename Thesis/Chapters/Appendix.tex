...
\chapter{Algorithms}
  ...
  \section{Compiled randomized benchmarking}
    ...
    \subsection{Optimizing the gate compilation algorithm}
      ...
      Using these $5$ optimizations the average gates per Clifford tuple has been calculated exactly for $n=5$ qubits within two hours. Furthermore, the average gates per Clifford has been approximated for up to $n=10$ Cliffords using random sampling. The values are shown in Figure~\ref{tab:gates per Clifford}.
