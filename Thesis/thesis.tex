  ...
  \chapter{Qubit characterization}
    ...
    \section{Time-domain measurements}
      ...
      \subsection{Tuning qubit frequency}
        Spectroscopy provides an estimate for the frequency of the qubit. However, even with pulsed spectroscopy, the accuracy is limited to roughly a megahertz. When the frequency of the driving tone $\omega_d$ is different from the qubit frequency $\omega_q$, the result is that the qubit precesses around the Z-axis, with a rate depending on the detuning $\Delta = \omega_d - \omega_q$. This property is exploited in a Ramsey measurement, where the qubit frequency can be inferred from the precession rate. A Ramsey measurement is performed by first applying a $X_{\pi/2}$ pulse, after which the qubit lies on the equator of the Bloch sphere. The state of the qubit then starts to precess around the equator, with a rate determined by the amount of detuning $\Delta$. After a certain delay time $\tau$, an $X_{-\pi/2}$ pulse is applied to the qubit.

        Ignoring effects such as decoherence and gate errors for the moment, if there were no precession, the qubit should be pulsed back exactly to its ground state. However, any detuning $\Delta$ causes the state of the qubit to deviate from its original position on the Bloch sphere, and so the second pulse does not exactly cancel the effect of the first pulse. in fact, after half a period the second pulse results in the qubit being in the excited state. After one full period, however, the qubit has returned to its original position on the equator, and so the second pulse will yet again return the qubit to the ground state.

        In a Ramsey measurement the delay time $\tau$ between the two pulses is swept, and the result should exhibit an oscillatory behaviour. The frequen

        \begin{itemize}
          \item Driving with detuning causes the qubit to precess.
          \item Relation between precession rate and detuning.
          \item Artificial detuning
