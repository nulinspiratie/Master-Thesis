  ...
  \chapter{Qubit characterization}
    ...
    \section{Part I: Continuous-wave measurements}
      ...
      \subsection{Scanning for qubits}
        ...
        The measurement to perform in order to find the qubit depends on the amount of detuning between the resonator and qubit, which can be estimated from powersweep measurements. If the amount of detuning is large compared to the coupling strength ($\Delta \gg  g$), the system is in the dispersive regime. In this case one commonly performs a two-tone spectroscopy to find the qubit's frequency.

        \subsubsection{Spectroscopy}
          As was explained in section~\ref{sec:resonator-scan} the resonator experiences a $2 \chi$ frequency shift dependent on the state of the qubit. In the case of a resonator capacitively coupled to the feedline, the transmission experiences a dip at the resonator's frequency. This dip will correspondingly shift when the qubit's state is swapped. The transmission at the resonator's dip when the qubit is in the ground state will be dependent on the state of the qubit (low when the qubit is in the ground state, high when the qubit is in the excited state). This is the property exploited in a two-tone spectroscopy measurement.
        ...
        \subsubsection{Tracked spectroscopy}
